...
const FRICTION_DISTANCE = 42; // slow the movement when coming close to the text
...
We have a wide range of beverages!`,
  ...
  useFrame((state, delta) => {
    ...
    //avoid negative value on Mac, so use Math.max
    const scrollOffset = Math.max(0, scroll.offset);

    let friction = 1;
    let resetCameraRail = true;
    // LOOK TO CLOSE TEXT SECTIONS (rail => make camera go close to the text when it's near)
    textSections.forEach(textSection => {
      ...
      if (distance < FRICTION_DISTANCE) {
        ...
        // reset rail when text is passed
        resetCameraRail = false;
      }
    })

    if (resetCameraRail) {
      // if no text is near, reset camera position to the center
    ...
    // seek the position of the timeline based on the scrollOffset
    ...
    // Make the cameraGroup look ahead on the curve
    ...
    // get the vector to be used for look at
    ...
    // get the angle of rotation on the curve we use the tangent of the point further ahead on the curve
    const tangent = curve.getTangent(lerpedScrollOffset + CURVE_AHEAD_AIRPLANE)
    // calculate the non lerp look at position to avoid our rotation to be affected by the smooth effect on the camera
    ...
    // as the camera follox the curve, we need to apply the inverse of the lookAt rotation to calculate the correct angle whatever the direction is
    let angle = Math.atan2(-tangent.z, tangent.x);
    angle = -Math.PI / 2 + angle;

    let angleDegrees = (angle * 180) / Math.PI; // convert angle to degrees
    ...
    // SET BACK ANGLE to radian
    angle = (angleDegrees * Math.PI) / 180;

    // then create the quaternion to rotate the plane smoothly with slerp
